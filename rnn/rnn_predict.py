...
argparser.add_argument('-w', '--weights', required=True,
...
def predictOnVideo(video_path, output_path, config, maxFramesToRunOn, detectionModel, keypoints_model, pose_config):
    ...
    detectionModel.full_model_inference.reset_states()
    ...
    minute_timer = time()
    detection_minute_time, keypoints_minute_time = 0, 0
    ...
    for i in tqdm(list(range(frames_to_predict))):
        ...
        if i % 1800 == 0 and i != 0:  # one minute timer
            ...
            print("Detection: %.3f | Keypoints: %.3f s" % (detection_minute_time,
                                                           keypoints_minute_time))
            detection_minute_time, keypoints_minute_time = 0, 0
        ...
        if i % skip_rate == 0:
            ...
            #     shovel_type=args.shovel_type,
            #     num_teeth=num_teeth,
            #     class_obj_threshold=class_obj_threshold
            # )
            activity, netout = detectionModel.predict(image)
            ...
            barplot_path = get_pred_bar_graph(netout[0, :],
                                       activity_labels.copy(), i)
            barplot_filepaths.append(barplot_path)
            image_buffer.append(image)
            pred_buffer.append((activity, netout))
            all_netouts.append(netout[0, :])
...
def _main_(args):
    ...
    set_keras_session(gpu_mem_fraction=0.3)

    tracker_model = TrackerModel(config, is_inference=True)
    pp.pprint(vars(tracker_model))
    tracker_model.full_model.load_weights(args.weights)

    #some_model = keras.models.load_model("/media/sf_N_DRIVE/randd/MachineLearning/"\
    #        "TeamMembers/Anuar/data/optical/teeth_activity/hydraulic_full/"\
    #        "output_folder/e2e_tiny_yolo/lstm_act__21_BbAndLstm_valLoss-0.0471.h5")
    #for layer1 in tracker_model.full_model.layers:
    #    for layer2 in some_model.layers:
    ...
    if args.input[-4:] in ['.mp4', '.avi', '.mov']:
        predictOnVideo(args.input, args.output, config, 9000, tracker_model, args.keypoints_model, args.keypoints_config)
